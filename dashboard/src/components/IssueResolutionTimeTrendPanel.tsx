...
import {
  ...
  type IssueResolutionTimeTrendDirection,
} from '@/lib/aggregate';

const DIRECTION_LABELS: Record<IssueResolutionTimeTrendDirection, string> = {
  increasing: '悪化傾向',
  decreasing: '改善傾向',
  flat: '横ばい',
};

const DIRECTION_STYLES: Record<IssueResolutionTimeTrendDirection, string> = {
  increasing: 'text-rose-400',
  decreasing: 'text-emerald-400',
  flat: 'text-sky-400',
};

const DIRECTION_DOT_STYLES: Record<IssueResolutionTimeTrendDirection, string> = {
...
export function IssueResolutionTimeTrendPanel({ runs }: { runs: RunRecord[] }) {
  ...
  // TimeToFirstPrTrendPanel と同様、単一点だと path が moveto 単独になり線が実質見えないため circle で補う。
  const singlePoint =
    points.length === 1
  ...
  return (
    <div
      ...
      data-testid="issue-resolution-time-trend-panel"
    >
      ...
      <svg
        ...
        role="img"
        aria-label="Issue生成からIssueクローズまでの解決時間の推移"
      >
      ...
      <p data-testid="issue-resolution-time-trend-latest" className="mt-2 text-[10px] opacity-50">
        直近解決: issue #{latest.issueNumber}（生成 iteration {latest.createdIteration} → クローズ iteration{' '}
        {latest.iteration}）
      </p>

      {signal === null ? (
        <p className="mt-3 text-xs opacity-50">
          解決済みissueが少なく、傾向（直近ウィンドウと直前ウィンドウの比較）はまだ判定できません。
        </p>
      ) : (
        <div data-testid="issue-resolution-time-trend-signal" data-direction={signal.direction}>
          <div className="mt-3 flex items-baseline justify-between">
            <span
              data-testid="issue-resolution-time-trend-direction"
            ...
            <span className="text-xs opacity-60">
              ...
              <span data-testid="issue-resolution-time-trend-recent-avg" className="font-semibold tabular-nums">
                {toMinutes(signal.recentAvgSec).toFixed(1)}分
              </span>
              {' / '}直前{signal.windowSize}件 平均{' '}
              <span data-testid="issue-resolution-time-trend-previous-avg" className="font-semibold tabular-nums">
