  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/5 p-5" data-testid="revision-size-curve-panel">
      <div className="flex items-baseline justify-between">
        ...
        <span
          className={`rounded px-2 py-0.5 text-xs tabular-nums ${SHAPE_COLORS[signal.shape]}`}
          data-testid="revision-size-curve-shape"
        >
      ...
      <ul className="mt-4 space-y-3">
        {buckets.map((b) => {
          const barPct = (b.avgReviseCycles / maxAvg) * 100;
          return (
            <li key={b.sizeBucket} data-testid={`revision-size-curve-row-${b.sizeBucket}`}>
              <div className="flex items-baseline justify-between text-sm">
                ...
                <span
                  data-testid={`revision-size-curve-stats-${b.sizeBucket}`}
                  className="tabular-nums opacity-60"
                >
                  平均revise {b.avgReviseCycles.toFixed(2)}回 / 中央値 {b.medianReviseCycles.toFixed(2)}回 (
                  {b.total}件, 平均{b.avgChangedLines.toFixed(0)}行)
                </span>
              </div>
      ...
      {signal.shape === 'insufficient-data' ? (
        <p className="mt-3 text-[10px] opacity-50">
          区分ごとの反復数が不足しているため、カーブ形状（加速/減速/比例）は未判定です。
        </p>
      ) : (
