...
import { timeToFirstPrTrend, timeToFirstPrTrendSignal, type TimeToFirstPrTrendDirection } from '@/lib/aggregate';

const DIRECTION_LABELS: Record<TimeToFirstPrTrendDirection, string> = {
  increasing: '悪化傾向',
  decreasing: '改善傾向',
  flat: '横ばい',
};

const DIRECTION_STYLES: Record<TimeToFirstPrTrendDirection, string> = {
  increasing: 'text-rose-400',
  decreasing: 'text-emerald-400',
  flat: 'text-sky-400',
};

const DIRECTION_DOT_STYLES: Record<TimeToFirstPrTrendDirection, string> = {
...
export function TimeToFirstPrTrendPanel({ runs }: { runs: RunRecord[] }) {
  ...
  // CycleTimeTrendPanel と同様、単一点だと path が moveto 単独になり線が実質見えないため circle で補う。
  const singlePoint =
    points.length === 1
  ...
  return (
    <div
      ...
      data-testid="time-to-first-pr-trend-panel"
    >
      ...
      <svg
        ...
        role="img"
        aria-label="Issue開始から初PR作成までの時間の推移"
      >
      ...
      {signal === null ? (
        ...
        <div data-testid="time-to-first-pr-trend-signal" data-direction={signal.direction}>
          <div className="mt-3 flex items-baseline justify-between">
            <span
              data-testid="time-to-first-pr-trend-direction"
            ...
            <span className="text-xs opacity-60">
              ...
              <span data-testid="time-to-first-pr-trend-recent-avg" className="font-semibold tabular-nums">
                {toMinutes(signal.recentAvgSec).toFixed(1)}分
              </span>
              {' / '}直前{signal.windowSize}反復 平均{' '}
              <span data-testid="time-to-first-pr-trend-previous-avg" className="font-semibold tabular-nums">
