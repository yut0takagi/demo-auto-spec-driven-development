...
export function OperatingHourSpectrumPanel({ runs }: { runs: RunRecord[] }) {
  ...
  const totalCount = spectrum.reduce((sum, b) => sum + b.count, 0);
  const maxCount = Math.max(...spectrum.map((b) => b.count));
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/5 p-5" data-testid="operating-hour-spectrum-panel">
      ...
      <p className="mt-1 text-[10px] opacity-50">
        ...
        （平日夜間・早朝・土日全時間帯）を夜間として時間帯ごとの件数を並べたもの。
      </p>
      ...
      <ul className="mt-4 flex h-24 items-end gap-0.5" data-testid="operating-hour-bars">
        {spectrum.map((b) => (
          <li
            ...
            title={`${b.hour}時 / ${b.count}反復 / ${CATEGORY_LABELS[b.category]}`}
          >
            <div
              className={`w-full ${CATEGORY_COLORS[b.category]}`}
              style={{ height: `${maxCount === 0 ? 0 : (b.count / maxCount) * 100}%` }}
            />
          </li>
        ))}
      </ul>
