...
// BuilderModelGateReasonCorrelationPanel と同じラベル集合。`Record<GateReasonCategory, string>` が
...
// VerdictTransitionPanel と同じラベル・色集合。
...
/** この lift 以上は「全体平均より明確に多く起きている」とみなす目安値 */
const LIFT_OVER_THRESHOLD = 1.2;
/** この lift 以下は「全体平均より明確に少ない」とみなす目安値 */
...
export function VerdictTransitionRootCausePanel({ runs }: { runs: RunRecord[] }) {
  ...
  return (
    <div
      ...
      data-testid="verdict-transition-root-cause-panel"
    >
      ...
      <p className="mt-1 text-[10px] opacity-50">
        lift = 到達側runがそのkindで持つ理由の占有率 ÷ 対象run全体での占有率。1.0が「平均通り」で、
        大きいほどそのverdict遷移で当該理由が過剰発生している（赤）。{LIFT_UNDER_THRESHOLD}
        以下は平均より明確に少ない（緑）。recovered/sustainedSuccessはgateReasonsが常に空のため対象外。
      </p>
          ...
          >
            ...
            <ul className="mt-2 space-y-1.5 pl-3">
              {row.cells.map((cell) => (
                <li
                  key={cell.category}
                  data-testid={`verdict-transition-root-cause-cell-${row.kind}-${cell.category}`}
                >
                  <div className="flex items-baseline justify-between text-xs">
                    <span className="opacity-70">{CATEGORY_LABELS[cell.category]}</span>
                    <span
                      data-testid={`verdict-transition-root-cause-lift-${row.kind}-${cell.category}`}
                      className="tabular-nums opacity-60"
                    >
                      {`lift ${cell.lift.toFixed(2)}x（${cell.count}件, 自${cell.withinKindSharePct.toFixed(0)}% / 全${cell.baselineSharePct.toFixed(0)}%）`}
                    </span>
                  </div>
                  <div className="mt-0.5 h-1.5 w-full overflow-hidden rounded-full bg-white/10">
                    <div
                      data-testid={`verdict-transition-root-cause-bar-${row.kind}-${cell.category}`}
